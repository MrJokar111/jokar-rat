...
def printbanner():
    ...
    for charset in range(0, 3):
        for pos in range(0, len(banner)):
            for i in range(0, len(banner[pos])):
                clr = f'\033[38;5;{txtcolor}m'
                char = f'{clr}{banner[pos][i][charset]}'
                final.append(char)
                cl += 1
                txtcolor = txtcolor + 36 if cl <= 3 else txtcolor
    ...
    haxorprint('Unleashed', 17)
    printmeta()

def haxorprint(string, color):
    clr = f'\033[38;5;{color}m'
    print(clr + string + '\033[0m')

def printmeta():
  print('Created By : xyz')




def print_meta():
	print(f'{META} Created by MR.Abdo Elgokar')
	print(f'{META} YOU Can Contact me: Telegram User: @Elgokar_1')
...
def main():
    ...
    if args.update:
        ...
        try:
            ...
            u = check_output(['git', 'pull', 'https://github.com/t3l3machus/Villain', 'main'], cwd=current_wd).decode('utf-8')
...
print('[INFO] Initializing required services:')
...
core = Core_Server()
core_server = Thread(target=core.initiate, name='team_server')
...
while time.time() < (timeout_start + 5):
    if netcat.listener_initialized:
        break
    elif netcat.listener_initialized is False:
        sys.exit(1)
else:
    sys.exit(1)
